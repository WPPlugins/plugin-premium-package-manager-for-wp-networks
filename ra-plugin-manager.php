...
function rapm_network_admin_plugin_actions( $actions, $plugin_file, $plugin_data, $context ) {
	...
	if( $context == 'all' && !$plugin_data['Network'] && !is_plugin_active_for_network( $plugin_file ) ) {
		...
		if( in_array( $plugin_file, $visible ) )
			$actions['ra_hide_plugin'] = '<a href="' . wp_nonce_url( rapm_network_admin_plugin_action_url( 'hide', $plugin_file, $context, $page, $s ), 'ra_hide-plugin_' . $plugin_file ) . '" title="' . __('Hide this plugin on the site plugin screens') . '">' . __('Make invisible') . '</a>';
		else
			$actions['ra_show_plugin'] = '<a href="' . wp_nonce_url( rapm_network_admin_plugin_action_url( 'show', $plugin_file, $context, $page, $s ), 'ra_show-plugin_' . $plugin_file ) . '" title="' . __('Show this plugin on the site plugin screens') . '">' . __('Make visible') . '</a>';
	}
	return $actions;
}
